...
/** Vordefinierte Agent-Pool-IDs. */
const AGENT_SPEC_IDS = ["ubuntu-latest", "windows-latest", "macOS-latest", "self-hosted"] as const;
...
export default function StageEditorPage() {
  ...
  const t = useTranslations("stageEditor");
  ...
  // Agent-Labels aus Übersetzungen ableiten
  const agentLabel: Record<string, string> = {
    "ubuntu-latest": t("agentUbuntu"),
    "windows-latest": t("agentWindows"),
    "macOS-latest": t("agentMacos"),
    "self-hosted": t("agentSelfHosted"),
  };
  ...
  const handleTaskInput = (taskIndex: number, key: string, value: string) => {
    const tasks = stage.tasks.map((task, i) =>
      i === taskIndex ? { ...task, inputs: { ...task.inputs, [key]: value } } : task
    );
    patchStage({ tasks });
  };

  const handleTaskNameChange = (taskIndex: number, name: string) => {
    const tasks = stage.tasks.map((task, i) => (i === taskIndex ? { ...task, name } : task));
    patchStage({ tasks });
  };
    ...
    <div className="min-h-screen">
      <AppBar
        ...
        rightSlot={
          <button
            onClick={saveAndGoBack}
            className="flex items-center gap-1.5 rounded-full bg-blue-600 px-3.5 py-1.5 text-sm font-semibold text-white active:opacity-70 transition-opacity"
          >
            <Check size={13} />
            {t("apply")}
          </button>
      ...
      <div className="px-4 pt-4 pb-3 border-b border-slate-800">
        <p className="text-base font-semibold text-slate-100 truncate">
          {stage.name || <span className="italic text-slate-500">{t("unnamedStage")}</span>}
        </p>
        <p className="text-xs text-slate-500 mt-0.5">{stage.agentSpec}</p>
      </div>
        ...
        {activeTab === "tasks" && (
          <div className="space-y-3">
            {stage.tasks.length === 0 && (
              <p className="py-6 text-center text-sm text-slate-500">{t("noTasks")}</p>
            )}
                  ...
                  {expandedTaskIndex === taskIdx && (
                    <div className="space-y-3 border-t border-slate-700/50 px-4 py-3">
                      ...
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-slate-400">{t("displayName")}</label>
                        <input
                          type="text"
                          value={task.name}
        ...
        {activeTab === "approvals" && (
          <div className="space-y-4">
            <ApprovalSection
              ...
              subtitle={t("approvalPreSubtitle")}
            ...
            <ApprovalSection
              ...
              subtitle={t("approvalPostSubtitle")}
        ...
        {activeTab === "agent" && (
          <div className="space-y-4">
            <div className="space-y-3 rounded-2xl border border-slate-700/60 bg-slate-800/40 p-4">
              <p className="text-[11px] uppercase tracking-wide text-slate-500">{t("sectionStage")}</p>
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-slate-300">{t("stageNameLabel")}</label>
                <input
                  type="text"
                  value={stage.name}
            ...
            <div className="space-y-2 rounded-2xl border border-slate-700/60 bg-slate-800/40 p-4">
              ...
              {stage.agentSpec === "self-hosted" && (
                <div className="mt-2 space-y-1">
                  <label className="text-xs text-slate-500">{t("poolNameLabel")}</label>
                  <input
                    type="text"
...
function TaskPickerModal({ taskDefs, isLoading, onSelect, onClose }: TaskPickerModalProps) {
  ...
  const [category, setCategory] = useState<string>(ALL_SENTINEL);

  const categories = useMemo(() => {
    const cats = Array.from(new Set(taskDefs.map((d) => d.category))).sort();
    return [ALL_SENTINEL, ...cats];
  }, [taskDefs]);
  ...
  const filtered = useMemo(() => {
    ...
    return taskDefs.filter((d) => {
      ...
      const matchesCategory = category === ALL_SENTINEL || d.category === category;
      return matchesSearch && matchesCategory;
    });
  }, [taskDefs, search, category]);

  return (
    <Modal open onClose={onClose} title={t("taskPickerTitle")}>
      <div className="space-y-3">
        {/* Suche */}
        ...
        <div className="max-h-80 divide-y divide-slate-800/60 overflow-y-auto rounded-xl border border-slate-700/60">
          ...
          {!isLoading && filtered.length === 0 && (
            <p className="px-4 py-6 text-center text-sm text-slate-500">{t("noTasksFound")}</p>
          )}
        ...
        <p className="text-center text-xs text-slate-600">
          {t("taskCount", { filtered: filtered.length, total: taskDefs.length })}
        </p>
      </div>
    </Modal>
  );
...
function ApprovalSection({
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-slate-700/60 bg-slate-800/40 p-4">
      ...
      {!config.isAutomated && (
        <div className="space-y-2">
          {config.approvers.length === 0 && (
            <p className="text-xs text-slate-500">{t("addApproverHint")}</p>
          )}
      ...
      {config.isAutomated && (
        <p className="text-xs text-slate-500">{t("autoApprovalHint")}</p>
      )}
    </div>
  );
}
