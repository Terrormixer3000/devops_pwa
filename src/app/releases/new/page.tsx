...
import { ArrowDown, ChevronLeft, ChevronRight, Loader, Plus, Rocket, Trash2 } from "lucide-react";
...
export default function NewReleasePipelinePage() {
  ...
  const t = useTranslations("createReleasePipeline");
  ...
  const createMutation = useMutation({
    mutationFn: () => {
      if (!vsrmClient || !settings || !draft) throw new Error(tc("noClient"));
    ...
    onError: (error) => {
      setSubmitError(extractErrorMessage(error, t("createError")));
    },
  });
  ...
  const CreateButton = (
    <button
      ...
      className="flex items-center gap-1.5 rounded-full bg-blue-600 px-3.5 py-1.5 text-sm font-semibold text-white active:opacity-70 transition-opacity disabled:opacity-40"
    >
      {createMutation.isPending ? <Loader size={13} className="animate-spin" /> : <Rocket size={13} />}
      {t("create")}
    </button>
  );
  ...
  if (!draft) {
    return (
      <div className="min-h-screen">
        <AppBar title={BackTitle} rightSlot={CreateButton} hideProjectChip />
        <PageLoader />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-2xl space-y-5 px-4 pb-8 pt-4">
        {/* Fehleranzeige */}
        {submitError && (
        ...
        <div className="space-y-4 rounded-2xl border border-slate-700/60 bg-slate-800/45 p-4">
          <p className="text-[11px] uppercase tracking-wide text-slate-500">{t("sectionGeneral")}</p>
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-medium text-slate-300">
              {t("descriptionLabel")} <span className="font-normal text-slate-500">({tc("optional")})</span>
            </label>
            <input
              type="text"
        ...
        <div className="space-y-3 rounded-2xl border border-slate-700/60 bg-slate-800/45 p-4">
          <p className="text-[11px] uppercase tracking-wide text-slate-500">{t("sectionArtifact")}</p>
          <p className="text-xs text-slate-500">
            {t("artifactHint")} <span className="text-slate-600">({tc("optional")})</span>
          </p>
          ...
          {pipelinesLoading ? (
            ...
            <div className="max-h-44 divide-y divide-slate-700/50 overflow-y-auto rounded-xl border border-slate-700 bg-slate-800/60">
              ...
              {!pipelines?.length && (
                <p className="px-3 py-2.5 text-sm text-slate-500">{t("noPipelinesFound")}</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="space-y-2 rounded-2xl border border-slate-700/60 bg-slate-800/45 p-4">
          <p className="text-[11px] uppercase tracking-wide text-slate-500">{t("sectionStages")}</p>
      ...
      >
        <div className="min-w-0 flex-1">
          ...
          <p className={`mt-0.5 text-xs ${hasManualApproval ? "text-yellow-500" : "text-slate-600"}`}>
            {hasManualApproval ? t("manualApprovals") : t("autoApprovals")}
          </p>
        </div>
